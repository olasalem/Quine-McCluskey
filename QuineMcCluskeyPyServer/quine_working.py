...
if (argExec):
	...
	data = json.loads(inputArg)
	try:
		minterms = map(int , data["minterms"].split(","))
...
if (argExec):
	...
	print "Variables:"
	try:
		maxPower = int(raw_input())
	except ValueError:
		if minterms is not None:	
			maxPower = ceil(log(max(minterms)+1,2))	

tempMax =  ceil(log(max(minterms)+1,2))
maxPower = max(maxPower, tempMax)
...
if (argExec):
	try:
		dontcares = map(int , data["dontcares"].split(","))
	except ValueError:
		dontcares = None
...
JsonImplicants = json.dumps(primeImplicants)
set1 = set()
set2 = set()
duplicates = set()
for v in primeImplicants:
...
try:
	...
	None
JsonPrimeImplicants = json.dumps(primeImplicants)
if (not argExec):	
	print "final PI:"
...
finalFunction = ""
finalFunction += "function "+ str(v).replace(']',')').replace('[','(').replace("'","")+ "= "
	...
	if "1" in finalOutput:print "1"
		...
		for i in range (0,len(finalOutput)):
			...
			if i is not len(finalOutput)-1:
				print " + "
		print " """
if "1" in finalOutput:finalFunction+= "1"
else:
	for i in range (0,len(finalOutput)):
		finalFunction += finalOutput[i]
		if i is not len(finalOutput)-1:
			finalFunction+= " + "
JsonFunction = json.dumps(finalFunction)
JsonArrayResponse = json.dumps({"implicants":JsonImplicants, "primeImplicants":JsonPrimeImplicants, "cover":JsonCover, "function":JsonFunction})
